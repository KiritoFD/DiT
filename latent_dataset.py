...
def _load_one(task):
    ...
    with Image.open(path) as im:
        return i, np.asarray(im.convert(mode), dtype=np.uint8)
...
class MCCDLatentDataset(Dataset):
    ...
    def _preload_all(self, num_workers=16):
        ...
        if self.load_canny and self.canny_root:
            self._cannys = np.empty((n, 256, 256), dtype=np.uint8)
        ...
        if self.load_skel and self.skel_root:
            self._skels = np.empty((n, 256, 256), dtype=np.uint8)
        ...
        if self.preload:
            ...
            if self.load_canny and self.canny_root:
                with Image.open(os.path.join(self.canny_root, f"{img_id}.png")) as c:
                    canny = c.convert('L')
                canny_t = (torch.from_numpy(np.asarray(canny, dtype=np.float32) / 255.0) > 0.5).float().unsqueeze(0)
            ...
            if self.load_skel and self.skel_root:
                with Image.open(os.path.join(self.skel_root, f"{img_id}.png")) as sk:
                    skel = sk.convert('L')
                skel_t = (torch.from_numpy(np.asarray(skel, dtype=np.float32) / 255.0) > 0.5).float().unsqueeze(0)
