...
train_ids, train_ca, train_ch = set(), set(), set()
train_ca_id, train_ch_id = {}, {}
...
print("train_top6 imgs:", len(train_ids), " (script,calli):", len(train_ca), " (script,char):", len(train_ch))
...
for sname in SCRIPT_NAME_TO_ID:
    sub = by_script[sname]
    print(f"  {sname}: {len(sub)} cand, {len(set(e['orig_char'] for _,e,_,_ in sub))} chars, "
          f"{len(set(e['orig_calli'] for _,e,_,_ in sub))} callis")