...
Creates:
  ...
  _smoke_data/data/imgs/final_imgs_256/{0..35}.png  (random noise-ish 256x256)
...
os.makedirs(os.path.join(ROOT, "data/imgs/final_imgs_256"), exist_ok=True)
...
for i in range(N_TRAIN + N_EVAL):
    arr = make_img(1000 + i)
    Image.fromarray(arr).save(os.path.join(ROOT, "data/imgs/final_imgs_256", f"{i}.png"))
    rows.append({
        "image_path": f"data/imgs/final_imgs_256/{i}.png",
...
with torch.no_grad():
    for i in range(N_TRAIN + N_EVAL):
        img = Image.open(os.path.join(ROOT, "data/imgs/final_imgs_256", f"{i}.png")).convert("RGB")
