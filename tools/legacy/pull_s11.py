...
复用 tools/evaluation/pull_monitor.py 的底层函数 (_ssh/_scp_dir/parse/merge_eval_json/
_pull_sample_dir/_strip_label/make_eval_poster 调用), 参数化为多实验:
...
REMOTE_CSV = f"{pm.REMOTE_BASE}/5script/show2_top6.csv"
...
def collect_series(series):
    ...
    latest = pm._ssh(
        f"find {pm.REMOTE_BASE}/5script/results/{series} -name log.txt 2>/dev/null "
        f"| xargs ls -t 2>/dev/null | head -1", timeout=20).strip()
    if not latest:
        return None, "", "", None, [], []
    ...
    show_steps = _strip(parts[3]).splitlines() if len(parts) >= 4 else []
    seen_steps = _strip(parts[4]).splitlines() if len(parts) >= 5 else []
...
def _exp_local_dirs(exp):
    ...
    return {
        ...
        "eval": os.path.join(base, "eval_samples"),
    }
...
def _regen_poster(exp, ckpt_dir, dirs, verbose=True):
    ...
    # 收集该 series 所有 eval_auto json 到临时目录
    ...
    try:
        ...
        if verbose:
            if r.returncode != 0:
                print(f"[poster] {tag} rc={r.returncode}: {r.stderr[:300]}")
            else:
...
def build_compare_dashboard(verbose=True):
    ...
    if len(exps_data) < 2:
        if verbose:
            print(f"[compare] 只有 {len(exps_data)} 个实验有数据, 对比 dashboard 暂缓")
        return None
    ...
    with open(out, "w", encoding="utf-8") as f:
        f.write(html)
    if verbose:
        print(f"[compare] {len(exps_data)} 实验 -> {out}")
    return out
...
METRICS.forEach((m, mi) => {
  ...
  const ds = DATA.map((d, i) => ({
    label: `${d.patch===2?'p2':d.patch===4?'p4':'p8'} (${d.name})`,
...
def run_once(verbose=True, do_poster=True):
    ...
    for local, remote_path in [(SHOW_CSV, f"{pm.REMOTE_BASE}/5script/show2_top6.csv"),
        ...
        if not os.path.exists(local):
            pm._ssh(f"cat {remote_path}", timeout=15)
