...
方案：用每个 glyph (script×char) 的 medoid data/skel/final_skeleton 作为空间字形条件。
检查：
  1. unique glyph 数
  2. 每 glyph 有多少张图（够不够取 medoid）
  3. 用 data/skel/final_skeleton 作为 glyph spatial 参考是否可行
"""
import csv, os, glob
...
# 检查 data/skel/final_skeleton 覆盖（能不能从训练图取骨架）
skel_ids = set(os.path.splitext(os.path.basename(p))[0] for p in glob.glob("data/skel/final_skeleton/*.png"))
img_id_of_sample = os.path.basename(rows[0]["image_path"])[:-4]
print(f"data/skel/final_skeleton 总图数: {len(skel_ids)}")
