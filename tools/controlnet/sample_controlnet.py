...
支持两种 skel 来源:
  1. GT skel: 从真实图片 ID 加载 (final_skeleton_d3/<id>.png)
  2. 标准字库 skel: 给定 character_id, 用标准字库生成的标准骨架
     (路径: std_glyphs/skel/<char_id>.png)

流程:
  1. 加载主模型 + ControlNet 权重
  2. 加载 skel 条件 (GT 或标准字库)
  3. DDIM 采样 (50步, CFG on callig/char, skel 始终提供)
  4. VAE decode latent → 像素图 (256×256)
  5. 保存结果 + skel 条件对照图
...
def decode_latent(vae_path, latent, device):
    """VAE decode latent → 像素图 (1,3,256,256) in [-1,1]."""
