...
2. **GPU 编码**：VAE encode 在 cuda 上批跑（用户要求，不再走 CPU 路径）。
   ...
   （VAE encode）走 GPU。
3. **shard 布局对齐 data/skel/final_skel_latents_fame**：同样 20 个 shard、按 img_id
...
------------------------
  python tools/build_fame_skel1px.py \
      --train-csv assets/train_fame.csv \
      --eval-csv assets/eval_fame_strict.csv \
      --img-root data/imgs/final_imgs_256 \
      --skel1-dir data/skel/final_skel1_fame \
      --latent-out data/skel/final_skel_latents_fame_1px \
...
def main():
    ...
    ap.add_argument("--train-csv", default="assets/train_fame.csv")
    ap.add_argument("--eval-csv", default="assets/eval_fame_strict.csv")
    ap.add_argument("--img-root", default="data/imgs/final_imgs_256")
    ap.add_argument("--skel1-dir", default="data/skel/final_skel1_fame")
    ap.add_argument("--latent-out", default="data/skel/final_skel_latents_fame_1px")
