# -*- coding: utf-8 -*-
"""build_data/skel/std_skel1_latents.py — 渲染标准字形 -> 1px 骨架 -> VAE latent (一条龙).

输出两件套:
  1) data/skel/std_skel1_latents_fame/  训练 shards (shard_XXXXX.npz, latents+img_ids)  <- s27 训练条件
  2) data/skel/skel_bank_std1.npz       推理骨架库 (keys: "楷|字", latents)            <- gradio zero-shot 直出

1px = skeletonize 后不做 dilation (与 data/skel/final_skel1_fame 一致).
...
def parse_args():
    ...
    ap.add_argument("--out-shards", default="data/skel/std_skel1_latents_fame")
    ap.add_argument("--out-bank", default="data/skel/skel_bank_std1.npz")
...
def main():
    ...
    # 2) shards 保存 (按训练 csv 行展开, 缺失行跳过 -> 与 data/latents/final_latents_fame 对齐)
