...
def main():
    ...
    # ---- 2) best ckpt ----
    runs = sorted(glob.glob("assets/results/s21_fame_flow_v2/2026*"))
    run_dir = runs[-1]
    hist = {}
    ...
    log("waiting for data/skel/final_skel_latents_fame shards ...")
    t0 = time.time()
    while len(glob.glob("data/skel/final_skel_latents_fame/shard_*.npz")) < 20:
    ...
    while time.time() - t0 < 30 * 3600:
        ...
        if not procs("train_controlnet"):
            ...
            break
        hist = {}
        for p in glob.glob("assets/results/ctrl_fame_v2/*/checkpoints/eval_auto_*.json"):
            try:
                d = json.load(open(p, encoding="utf-8"))
                hist[int(d["step"])] = float(d["ssim"])
            except Exception:
        ...
        json.dump(history, open("assets/fame_ctrl_watchdog.json", "w"),
