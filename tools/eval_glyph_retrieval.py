...
用法（远程）
-----------
  python tools/eval_glyph_retrieval.py --build-csv assets/train_fame.csv \
      --query-csv assets/eval_fame_strict.csv --img-root data/imgs/final_imgs_256 \
...
def load_dino(device):
    ...
    if not ckpt:
        raise RuntimeError("未找到本地 DINOv2 ckpt（data/pretrained/dinov2_vits14_pretrain.safetensors）")
...
def main():
    ...
    ap.add_argument("--out", default="assets/glyph_retrieval_calib.json")
    args = ap.parse_args()
