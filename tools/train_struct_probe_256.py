...
不需要额外 cache 打包。skel/canny GT 已存在于 final_canny/ data/skel/final_skeleton/ (从图算好的)。
...
用法:
  /opt/conda/bin/python tools/train_struct_probe_256.py \
      --csv assets/train.csv --epochs 8 --batch-size 64
"""
import os
import sys
import time
...
def main():
    ...
    ap.add_argument("--csv", default="assets/train.csv")
    ap.add_argument("--latent-shards-dir", default="final_latents")
    ap.add_argument("--canny-root", default="final_canny")
    ap.add_argument("--skel-root", default="data/skel/final_skeleton")
    ap.add_argument("--out-dir", default="assets/results/structnet256")
