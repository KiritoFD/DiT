...
输出:
  assets/train_fame.csv              # 合并书家名后的训练表 (原 id 空间, 兼容 s20 warm-start)
  assets/eval_fame_strict.csv        # 组合泛化 eval (书家要素+字要素覆盖, (书家,字)组合未见过)
  data/latents/final_latents_fame/shard_*.npz      # img VAE latents (f16 4x32x32)
  fame.npz                            # 全量合并版 (latents f16 + img_ids)
  data/skel/final_skel_latents_fame/shard_*.npz # skel latents (ctrl 条件)
...
def main():
    ...
    for csvp in ["assets/train_mid_clean.csv", "assets/train_top6.csv",
                 "assets/train_3top30_nobeike.csv", "assets/train.csv"]:
    ...
    next_ch = 0
    dino_idx = json.load(open("data/pretrained/dino_embeddings/glyph_dino_index.json", encoding="utf-8"))
    ...
    def _check_ink(img_id):
        p = f"data/imgs/final_imgs_256/{img_id}.png"
    ...
    for e in entries:
        ...
        out_rows.append({
            "image_path": f"data/imgs/final_imgs_256/{e['img_id']}.png",
    ...
    with open("assets/train_fame.csv", "w", encoding="utf-8", newline="") as f:
    ...
    os.makedirs("data/latents/final_latents_fame", exist_ok=True)
    BS = 48
    ...
    for s in range(NSHARD):
        ...
        np.savez_compressed(f"data/latents/final_latents_fame/shard_{s:04d}.npz",
    ...
    with torch.no_grad():
        for i in range(0, len(out_rows), BS):
            ...
            for r in batch:
                ...
                Image.fromarray(a3, "L").save(f"data/skel/final_skel3_fame/{iid}")
                sks.append(a3)
    ...
    os.makedirs("data/skel/final_skel_latents_fame", exist_ok=True)
    for s in range(NSHARD):
        lo, hi = s * per, min((s + 1) * per, len(out_rows))
        np.savez_compressed(f"data/skel/final_skel_latents_fame/shard_{s:04d}.npz",
    ...
    for e in cand:
        ...
        if len(eval_rows) >= 500:
            break
    with open("assets/eval_fame_strict.csv", "w", encoding="utf-8", newline="") as f:
        w = csv.DictWriter(f, fieldnames=list(out_rows[0].keys()))
        w.writeheader()
