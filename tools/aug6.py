...
流程:
  Phase A (CPU, 多进程): 对 (script,char,calli) 组合中样本数 <6 的组合,
     生成 (6-n) 个增强变体 → 保存 PNG 到 data/imgs/final_imgs_mid_clean/{new_id}.png,
     同时写 aug_meta.csv (new_id,script,char,calli,calli_id,char_id,glyph_id,script_id).
  Phase B (GPU): 读取 data/imgs/final_imgs_mid_clean/*.png, sd-vae-ft-ema 编码 (scaling=0.18215)
     → 临时 latent shard /tmp/mid_clean_tmp/.
  Phase C (merge): 合并原 latent (只保留清洗后 csv 的 img_id) + 增强 latent → data/latents/final_latents_mid_clean/,
     写 train_mid_clean.csv (清洗后原行 + 增强行).

输入: assets/train_3top30_common.csv (GB2312 一级+二级 常用字, 23597 行)
增强算子 (每个变体随机组合):
...
def phase_c(args, rows, aug_meta):
    """合并原 latent + 增强 latent → data/latents/final_latents_mid_clean + CSV.
    ...
    with open(args.out_csv, "w", encoding="utf-8", newline="") as f:
        ...
        for new_id, script, char, calli, caid, chid, gid, sid in aug_meta:
            w.writerow({
                "image_path": f"data/imgs/final_imgs_mid_clean/{new_id}.png",
...
def main():
    ...
    ap.add_argument("--train-csv", default="assets/train_3top30_common.csv")
    ...
    ap.add_argument("--out-imgs", default="data/imgs/final_imgs_mid_clean")
    ap.add_argument("--out-latent", default="data/latents/final_latents_mid_clean")
    ap.add_argument("--out-csv", default="assets/train_mid_clean.csv")
