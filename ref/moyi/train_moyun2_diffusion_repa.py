...
import argparse
import logging
from moyun.moyun_2 import DiT_models
# from utils.diffusion import create_diffusion
from utils.REPA_diffusion import create_diffusion
...
def main(args):
    ...
    if rank == 0:
        ...
        if args.resume is not None:
            experiment_index = args.resume_index
    ...
    if (args.without_t == 1):
        print(f"{args.without_t=}")
    # diffusion = RF(
    #     without_t=(args.without_t == 1)
    # )
    vae = AutoencoderKL.from_pretrained(f"/mnt/ssd/model/image/sd-vae-ft-{args.vae}").to(device)
    ...
    logger.info(f"{args.feature_path=}")
    ...
    try:
        for epoch in range(args.epochs):
            ...
            loss_epoch = 0
            for image, edge, skeleton, y, stroke, calligrapher_feature, font_feature, charactor_feature in loader:
                # print(y.shape)
                ...
                y = y.to(device)
                stroke.to(device)
                if args.use_12channel == 0:
                    # 假设你的模型内部 REPA 期待的尺寸是 [B, C, H_latent, W_latent]
                    # 或者根据报错中的 '4'，我们动态造一个对应的 tensor
                    # 这里 C 的大小需要对应你的 feature 原始通道数（假设是 calligrapher_feature.shape[1]）
                    
                    # 方案 1：强制将加载的 feature 调整到模型想要的尺寸 (推荐，因为保留了 batch_size 和 通道数)
                    # 假设报错提示模型输出的是 4x4，而你输入的是 16x16
                    target_size = 4 
                    target_channels = 512
                    pseudo_feature = torch.zeros((calligrapher_feature.shape[0], target_channels, target_size, target_size), device=device)
                    calligrapher_feature = pseudo_feature
                    font_feature = pseudo_feature
                    charactor_feature = pseudo_feature

                feature_dict = {
                    "calligrapher": calligrapher_feature.to(device),
                    "font": font_feature.to(device),
                    "charactor": charactor_feature.to(device),
                ...
                # print(f"{x.shape=}")
                # print(calligrapher_feature.shape, font_feature.shape, charactor_feature.shape)
                # print()
                ...
                if train_steps % args.ckpt_every == 0 and train_steps > 0:
                    if rank == 0:
                        ...
                        logger.info(f"Saved checkpoint to {checkpoint_path}")
                    dist.barrier()
    except KeyboardInterrupt:  # 如果是命令行ctrlC停止运行，则先保存一次模型
        pass
    # 记录最终的一次模型
    if rank == 0:
        checkpoint = {
...
if __name__ == "__main__":
    ...
    parser.add_argument("--resume-index", type=int, default=None)  # 从旧的pt训练记一下step
    ...
    parser.add_argument("--without-t", type=int, default=0)
    parser.add_argument("--use_black_white_mse_loss", type=int, default=0)
    parser.add_argument("--use_grey_mse_loss", type=int, default=0)
    parser.add_argument("--calligrapher-x", type=float, default=0)
    parser.add_argument("--font-x", type=float, default=0)
    parser.add_argument("--charactor-x", type=float, default=0)
