#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""gpu_batch_eval_v2.py — 高效 GPU 批量评测。

核心优化:
1. fp16 推理 (模型+VAE)
2. 全部 455 张噪声+条件一次性放 GPU, 50 步 DDIM 全程零 CPU-GPU 传输
3. 手写 DDIM 循环 (不用 ddim_sample_loop 封装, 消除 Python 开销)
4. 50 步采完后批量 VAE decode
5. 最后一次性 .cpu() 传回, 落盘 + CPU 算指标
6. CFG: 455→910 forward, 分 2-3 大 batch per step
"""
...
def ddim_sample_all(model, conds, device, n_steps, cfg_scale, seed,
                    lc, ls, fwd_batch=256):
    """手写 DDIM: 全部样本同时在 GPU 上, 分大 batch per step, 零 CPU-GPU 传输。
    返回 fp32 latent (n, lc, ls, ls) on CPU."""
    ...
    with torch.no_grad():
        for ti, t_val in enumerate(timesteps):
            ...
            # 分大 batch 做 forward_with_cfg (内部会 cat cond+uncond → 2×batch)
            outputs = []
