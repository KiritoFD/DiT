...
# ── 扩散对象 ─────────────────────────────────────────────────────────────────
def build_diffusion(steps, diffusion_type="ddpm"):
    """Build the diffusion sampler for eval. steps: int (Euler/DDIM count)."""
...
def sample_latents(model, diffusion, noise, conds, cfg_scale, batch, device,
    ...
    skel        : optional (N,1,256,256) structural condition (ControlNet path).
...
def run_pair_eval(model, vae, diffusion, cache, device, step, checkpoint_dir,
    ...
    skel_arg = skels if with_skel else None
    ...
    n_saved = decode_and_save(vae, latents, sf, out_dir, tag,
                              gts=gts_all, skels=skels if with_skel else None,
                              vae_batch=vae_batch)
