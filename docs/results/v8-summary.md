...
| base | ssim | std | lpips | mse |
|---|---|---|---|---|
| s21 | 0.4764 | 0.1141 | 0.4448 | 1.0725 |
| s30 | 0.4934 | 0.1077 | 0.4419 | 0.9867 |
| **v8a** | **0.5182** | **0.1037** | **0.4139** | 1.0007 |

趋势：质量 ↑（ssim/lpips）且一致性 ↑（std 降）—— v8a 全面占优。

### 维度 3：后训练利用度（base → ctrl 增益）

| 模型 | ctrl | base | 增益 |
|---|---|---|---|
| v8b ctrl | 0.7641 | 0.5182 | +0.246 |
| v8c repa | 0.7674 | 0.4702 | +0.297 |

**强 base + ctrl 增益更大**（v8b 强 base 0.5182 的增益 beat 弱 base）—— 印证"好基模让后训练更有效"，但 v8c 的 +0.297 主要来自 REPA 本身。

### 维度 4（⭐ 关键）：字符区分度（DINO 特征能分开不同的字吗？）

来自 `research_dino_out.txt` + `dino_probe.json`：

- **同字跨书体检索 top-1 = 1.97%**（random ≈ 0.02%，仅比随机好 ~1.95pp）
- **形近字特征余弦极高**：土/士 0.986、王/玉 0.975、己/已 0.979、牛/午 0.966——但对应**骨架 IoU 仅 0.01-0.07**（结构其实不同）
- **Spearman(特征余弦 vs 骨架IoU) = +0.11**（768）—— DINO 的"字形相似"与"结构相似"几乎不相关

**诊断**：DINO 特征区分字符用的是**全局外观统计**（墨迹形状、笔画粗细密度），不是**拓扑结构**（笔画连通、起收笔位置）。土/士外观近 → DINO 视为同字 → 基模的 char 条件几乎无法区分它们 → 生成"土"时实际按"土∪士"的平均形态生成。

**预处理消融**（`dino_centered.json`，用检索率衡量）：
| 变体 | 跨书体 top-1 检索 |
|---|---|
| raw | 1.97% |
| **per_script_center** | **2.73%**（+39% 相对）|
| no_pc1 | 2.27% |
| pca200 | 1.87% |

→ **per-script centering 是唯一被量化证明有效的字符区分度增强**（有效秩 34.1→57 的另一面）。

### 我们基模差在哪（结论）

1. **char 条件本质上是"半盲"的**：字符区分度 1.97% 意味着 35130 字表里大部分字在特征空间不可分——**基模学不出"这个字"**，只能生成"这类形状的字"。s30/v8a 的 mlp proj 已在尽力重组，但重组的素材（低秩表）本身信息不够。
2. **提升路径排序**（按证据强度）：
   - ✅ **per-script centering**（检索 1.97→2.73%，秩 34→57，已量化）—— v9 预训练可直接用
   - ⚠️ 字形增强 / 多原型（逻辑可行未量化）
   - ❌ vitb14 教师（秩分析 768 vs 384 只多 8 维，判定不值）
   - ❌ ln_only 直通（s28 实测差 8.2%，已证伪）
3. **对后训练的含义**：ctrl 阶段 skel 条件救回了"结构"（+0.246 增益），但 base 的 char 盲区是**跨阶段累积的**（ctrl 也继承）—— 真正要突破的是**给 char 条件更可分的表征**（centering + 更结构化的字形特征），而不是再调后训练配方。

### 6.5b ⭐ DINO 表征增强系统性证伪（09-04，避免重蹈覆辙）

> 所有"加工 DINO 取出更好 char embedding"的路线**实测全部关闭**——今天的系统验证：

| 方向 | 实测 | 判定 |
|---|---|---|
| **骨架跑 DINO**（skel 图→vits14）| 形近字 cos 仍 0.90-0.96（土/士 0.965），有效秩 36 < 字形图 65 | ❌ DINO 不擅长 1px 线图 |
| **per-script centering 真实图检索** | top-1 **0.40%** < raw 1.20%（表内秩↑但真实可分性↓）| ❌ 校准推翻表内指标 |
| **检索式字形准确率**（真实图 query, DINO CLS NN）| raw **1.20%** top-1 —— **DINO 能力上界** | ⚠️ 天花板 |
| vitb14 教师 | 秩 768 vs 384 只多 8 维 | ❌ |
| **解冻 char 表**（s22：只训字 embedding）| 0.4622 < s21 冻结 0.4670 | ❌ 51k 数据学不动 35130 表 |

**总结论**：瓶颈是 **DINO 对书法字形不可分（真实图检索 1.2% 上界）+ 数据量太小（51k/35130 字 ≈ 1.4 样本/字）**。v8a 现配方（DINO init + mlp proj + freeze + centering + OT）已是该瓶颈下的最优组合，**继续在 DINO 表上加工预期无收益**。真正突破需引入**字形结构先验**（骨架拓扑特征 / 可学习结构编码），而不是 DINO 表征加工。

### 6.5c ⭐ 骨架信号的判别性剖析（09-04，含用户关键质疑）

> 理由：骨架 latent（sd-vae 4×32×32）是"有结构信息"的信号，评估它能否做 char 条件。

**骨架 latent vs DINO 的区分度（为什么 VAE 更好）**：
| 指标 | 骨架 VAE latent | 字形 DINO CLS | 骨架 DINO CLS |
|---|---|---|---|
| raw 维度 | 4096（4ch×32×32 空间网格）| 768（全局）| 384（全局）|
| 有效秩 90% | **~630**（每通道 148-172）| 65 | 36 |
| 同字跨书家检索（样本级）| **17%** | 1.2% | — |
| 形近字 cos | 0.97（样本级）| 0.79-0.99 | 0.90-0.96 |

**VAE 更好的原因**：sd-vae 是逐位置卷积重建编码器，latent 网格保留**空间拓扑**（士的横位差 → 特定网格激活不同）；DINO CLS 是全局语义池化，把空间信息**平均成外观统计**。书法字形判别是空间拓扑任务 → VAE 结构天然占优（有效秩 17×）。

**但用户质疑成立：骨架 latent 不能直接做 char 条件**——
1. **字级原型分析**：同字跨书家平均做原型后，检索 top-1 = **0%**（3050 字原型在 latent 空间"拓扑密堆"，最近邻必错）；同字 vs 异字 cos 仅 0.776 vs 0.756（信噪比 0.02）——骨架 latent 是**逐样本的图像压缩**，不是字身份干净编码
2. **与 ctrl 冗余**：第二阶段（skel-ctrl）的输入条件**就是该骨架 latent**——base 的 char 条件若也用它，两阶段条件同源冗余，模型无法区分"生成这个字的骨架"vs"按这个骨架生成"
3. 形近字原型 cos 0.84-0.91（样本级 0.97）证明**原型的书家噪声平均有帮助**，但信号仍弱

**结论**：骨架 latent 做 char 条件**不可行**（样本级泄漏 + 判别弱）；DINO patch tokens（空间保留的 256×384）**待测**（v8e 后补测）——若 patch 级对齐检索显著高于 CLS 的 1.2%，则为"空间正确 + 非泄漏"的新候选。
