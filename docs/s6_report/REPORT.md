...
## 9. S7 爬升实验中期结果:反而恶化(08-21 20:10)

实验 A(b8all,从 diffonly@195000 出发、struct 权重 20000 步线性爬升)的 eval100 曲线呈**单调恶化**:

| step | 微调步数 | struct 权重比例 | MSE | SSIM |
|---|---|---|---|---|
| 195000(基线) | 0 | 0% | 0.432 | 0.732 |
| 200000 | 5000 | 25% | 0.535 | 0.652 |
| 205000 | 10000 | 50% | 0.665 | 0.577 |
| 210000 | 15000 | 75% | 0.728 | 0.556 |
| 215000 | 20000 | **100%** | 0.759 | 0.546 |
| 220000 | 25000 | 100% | 0.779 | 0.540 |
| 225000 | 30000 | 100% | 0.786 | 0.538 |

**关键观察**:
1. **恶化与爬升权重同步单调**,且权重仅 25% 时就已显著回退——不是"加得太猛"的问题,**任何非零权重的 pixel 结构损失都会立即触发退化**;
2. **X0Lat 再次爆炸**:raw 从 diffonly 的 ~1-2 飙到 37-50,x0 流形漂移随爬升即时出现;
3. Diff 损失本身保持低位(~0.10)——扩散损失感知不到 x0 漂移,这正是漂移能持续恶化的原因;
4. 注意混淆因素:fresh-scheduler 把 LR 重置回高位(当前 1.32e-04),单纯续训也可能造成部分回退;但"恶化斜率与 struct 权重爬升同步"强烈指向结构损失是主因;
5. 按 early-stop 规则(best=200000 的 0.652,patience 6),若 230000 点仍无改善将触发早停,自动切换到实验 B(b32sub8)。

**结论修正**:结合第 5 节机理分析,"缓慢加入"并不能挽救 pixel 空间结构损失——只要它通过 VAE decoder 反传梯度,x0 就会漂出流形。后续方向应转向 **latent 空间结构约束**(不经过 decoder)或把结构信息作为**条件输入**而非损失。

---

## 10. 根因诊断:结构损失实现存在 3 个致命缺陷(08-21 排查)

逐行核查 `src/losses.py` 与 `src/train.py` 后,确认结构损失的"副作用"很大程度来自**实现 Bug**,而非结构监督本身不可用:

### 缺陷 1:SkeletonLoss 在"谋杀"笔画血肉(与笔画宽度冲突)
`skel_gt` 是 Zhang-Suen 细化后的 **1px 中心线**(`gen_canny_skel.py`);旧版用 `max_pool3x3` 膨胀仅覆盖中心线周围 1px,然后 `bg_mask=1-膨胀` 把**所有不在中心线 3px 内的墨水都当背景惩罚**(`off_skel = mean(bg_mask * ink)`)。
毛笔笔画通常 **10~30px 宽** → 笔画主体全被算进"背景",损失在**逼模型抹掉笔画血肉**。与 GT 匹配的扩散损失两头拉扯 → 矛盾点上崩溃成彩噪。

### 缺陷 2:SobelCannyLoss 归一化失真 + 跨域硬匹配
- 逐图 `grad/grad_max` 归一化:早期模糊 x0 的一点噪点被放大成"最强边缘",梯度尺度失真;
- 用连续梯度场 L1 拟合 **1px 二值 Canny 线**,等于逼模型消灭抗锯齿/晕染。

### 缺陷 3:没有噪声步门控(t-gating)
latent 版结构损失有 `latent_struct_max_t=500`,**pixel 版却对所有 t 都施加**。高噪声步(t~1000)的 x0 预测本来就是糊,强迫它"提前成像"会让 x0 整体漂出 VAE 流形——这正是 **X0Lat 从 ~2 爆到 37-50**、SSIM 崩塌的直接元凶。

### 修复(已实现 `use_struct_loss_v2` + `struct_max_t`,默认关闭,向后兼容)
1. **SkeletonLossV2**: 只做正向牵引(recall: 骨架上必须有墨),**不惩罚骨架外的墨水**,笔画粗细交给扩散损失决定;
2. **EdgeGradientLoss**: 预测图与 **GT 原图**的 Sobel 梯度幅值场直接 L1 匹配(不做逐图归一、不做二值匹配),保留边缘锐度与灰度过渡;
3. **struct_max_t=500**: 只在低噪声步施加结构损失。
数值健全性验证:SkeletonLossV2 自洽=0 / 缺墨=1.0;EdgeGradientLoss 自洽=0 / 错位>0 ✓。

### 结论修正
S7 的"恶化随权重爬升"不是结构监督没有价值,而是**这 3 个 Bug 让损失在反向"毁图"**。修复版实验 S8(b8all→b32sub8,同 S7 设计:diffonly@195000 起步 + 20000 步爬升 + fresh-scheduler)待跑,用于验证修复后结构监督能否带来增量。
