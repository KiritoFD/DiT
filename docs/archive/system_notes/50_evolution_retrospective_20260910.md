...
## T10 · 2026-09-11：Sp 过拟合观察 → **骨干改回 S/2 重训**

**观察（sty32 Sp/2 @202k/250k）**：

| 指标 | 轨迹 | 判读 |
|---|---|---|
| **seen**（记忆集） | 0.522@100k → 0.544@120k → 0.6175@175k → **0.6690@197.5k** | **140k 后陡涨，记忆化爬升段** |
| **strict**（新字） | 0.5355@100k → 0.5500@150k → **0.5518@190k** | 平台 0.550-0.552，不再随 seen 提升 |

→ 判定 **Sp/2（59M）相对 fame3（28k）过拟合**：容量吃记忆而非泛化。

**处置（07:50）**：停训 + 回 **S/2（32.7M）** 从零重训，**唯一变量 = 骨干尺寸**：
同样式 token 配方（n=32 / xattn×12 / 增强 fame_e 113,540 / lr 5e-5 cosine / 200k / eval_cfg 2.0）。
跑速 5.56 sps（Sp 为 4.09），显存 14.5G。判据：strict 是否突破 0.552 平台、
seen 陡涨是否推迟或减弱。
