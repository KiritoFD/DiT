...
|---|---|---|---|---|
| s21（基线 best） | 27500 | **0.4664** | 0.4430 | — |
| s21（自身继续训） | 30000 | 0.4670 | 0.4416 | — |
| **s22（只训 char embedding）** | 30000 | **0.4622** | 0.4469 | 1.0389 |

s22 从 s21@27500 resume、续训 2500 步（仅 char 分支可训）后：
**SSIM 0.4664 → 0.4622，下降 0.0042**，同时 LPIPS 变差（0.4430 → 0.4469）。

train loss 全程持平在 0.348–0.353（与 s21 收尾一致），没有可辨识的下降。

**判读**：解冻 char embedding **无益，且略微有害**。

这与 §2 的探测一致：DINO CLS 本身缺少字符身份信息，解冻后模型面对的是
「每字仅 10.8 个样本 × 384 维自由参数」的严重过拟合局面——
它学到的更可能是训练集噪声，而不是可泛化的字符表示。

**因此瓶颈不在「表被冻结」，而在「这个特征本身不含字符身份信息」。**
解冻一个信息量不足的表，只会引入过拟合自由度。

**结论：应转向 §6 的空间化条件，而不是继续在 embedding 表上做文章。**
