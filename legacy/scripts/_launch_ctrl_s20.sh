...
tmux new-session -d -s ctrl_s20 "export PYTORCH_CUDA_ALLOC_CONF=max_split_size_mb:128; cd /root/Workspace/xy/DiT && /opt/conda/bin/python -m src.train.legacy.train_controlnet --config src/train/configs/s20_ctrl_skel_flow_v2.json > /tmp/s20_ctrl_train.log 2>&1"
sleep 15
echo "--- tmux ---"
tmux ls 2>/dev/null
echo "--- proc ---"
