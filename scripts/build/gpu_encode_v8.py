...
"""gpu_encode_v8.py — 建 v8 新资产集 (不覆盖旧文件):
  ...
  - 打补丁写入新 npz (data/fame_clean_v8.npz) 与新 shards
  - 生成 v8 csv: image_path 指向 data/imgs/final_imgs_fame_v8/
...
def make_v8_csv():
    # 生成 v8 csv: image_path 全部指向 data/imgs/final_imgs_fame_v8/
    for src_csv, dst_csv in [
        ...
        with open(dst_csv, 'w', newline='', encoding='utf-8') as f:
            ...
            for r in rows:
                ...
                r['image_path'] = 'data/imgs/final_imgs_fame_v8/%d.png' % iid
                w.writerow(r)
...
if __name__ == '__main__':
    ...
    # ---------- 1) copy 旧资产 -> v8 ----------
    copy_tree('data/imgs/final_imgs_fame_clean', 'data/imgs/final_imgs_fame_v8')
    copy_tree('data/latents/final_latents_fame_clean', 'data/latents/final_latents_fame_v8')
    copy_tree('data/skel/final_skel1_fame', 'data/skel/final_skel1_fame_v8')
    copy_tree('data/skel/final_skel3_fame', 'data/skel/final_skel3_fame_v8')
    copy_tree('data/skel/final_skel_latents_fame', 'data/skel/final_skel_latents_fame_v8')
    copy_tree('data/skel/final_skel_latents_fame_1px', 'data/skel/final_skel_latents_fame_1px_v8')
    # 新 npz: copy 旧 fame.npz
    if not os.path.exists('data/fame_clean_v8.npz') and os.path.exists('fame.npz'):
        shutil.copy2('fame.npz', 'data/fame_clean_v8.npz')
        print('copied fame.npz -> data/fame_clean_v8.npz')
    make_v8_csv()
    if args.copy_only:
    ...
    if changed[0] not in [int(x.split('/')[-1][:-4]) for x in os.listdir('data/imgs/final_imgs_fame_v8')]:
        pass  # 目录存在性检查
    missing = [i for i in changed if not os.path.exists('data/imgs/final_imgs_fame_v8/%d.png' % i)]
    print('changed missing in v8 GT:', len(missing))
    if missing:
        # 从最终清洗图补
        for i in missing:
            if os.path.exists('data/imgs/final_imgs_fame_clean/%d.png' % i):
                shutil.copy2('data/imgs/final_imgs_fame_clean/%d.png' % i, 'data/imgs/final_imgs_fame_v8/%d.png' % i)
        print('backfilled missing from data/imgs/final_imgs_fame_clean')
    ...
    vae = AutoencoderKL.from_pretrained('data/pretrained/sd-vae-ft-ema').eval().to(device)
    ...
    for idx in range(0, len(changed), batch):
        ...
        # GT 图 (v8 目录)
        xb = torch.stack([tf(Image.open('data/imgs/final_imgs_fame_v8/%d.png' % i).convert('RGB')) for i in ids_b])
        z_img = enc(xb)
        # skel PNG 重算 + 写 v8 目录
        sk3_arr = np.zeros((len(ids_b), 256, 256), np.uint8)
        sk1_arr = np.zeros((len(ids_b), 256, 256), np.uint8)
        for k, iid in enumerate(ids_b):
            p = 'data/imgs/final_imgs_fame_v8/%d.png' % iid
            ...
            Image.fromarray(s3, 'L').save('data/skel/final_skel3_fame_v8/%d.png' % iid)
            Image.fromarray(s1, 'L').save('data/skel/final_skel1_fame_v8/%d.png' % iid)
        ...
        x1 = torch.stack([tf(Image.open('data/skel/final_skel1_fame_v8/%d.png' % i).convert('RGB')) for i in ids_b])
        z1 = enc(x1)
        for k, iid in enumerate(ids_b):
            img_lat[iid] = z_img[k]
            sk3_lat[iid] = z3[k]
    ...
    # ---------- 4) 打补丁写入 v8 ----------
    n = patch_npz('data/fame_clean_v8.npz', img_lat, 'img')
    n += patch_shards('data/latents/final_latents_fame_v8', img_lat, 'img shards')
    n += patch_shards('data/skel/final_skel_latents_fame_v8', sk3_lat, 'skel3 shards')
    n += patch_shards('data/skel/final_skel_latents_fame_1px_v8', sk1_lat, 'skel1 shards')
    ...
    for iid in sample:
        ...
        x = tf(Image.open('data/imgs/final_imgs_fame_v8/%d.png' % iid).convert('RGB'))[None]
        z = enc(x)[0]
        d = np.load('data/latents/final_latents_fame_v8/shard_%04d.npz' % (iid // 3000))  # 近似 shard 分桶, 下面用真实查询
        # 真查询: 扫 shards 找 id
        for sh in sorted(glob.glob('data/latents/final_latents_fame_v8/shard_*.npz')):
            dd = np.load(sh)
    ...
    bad1 = 0
    for iid in sample:
        z1n = enc(tf(Image.open('data/skel/final_skel1_fame_v8/%d.png' % iid).convert('RGB'))[None])[0]
        for sh in sorted(glob.glob('data/skel/final_skel_latents_fame_1px_v8/shard_*.npz')):
            dd = np.load(sh)
            if iid in dd['img_ids']:
                idx = list(dd['img_ids']).index(iid)
