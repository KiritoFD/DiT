#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""把 eval/show5/seen5 CSV 的 image_path 列从 final_images/ 改成 data/imgs/final_imgs_256/。
...
for name in ["eval100_top30_clean.csv", "show5_top30.csv", "seen5_top30.csv"]:
    ...
    for r in rows:
        if r["image_path"].startswith("final_images/"):
            r["image_path"] = r["image_path"].replace("final_images/", "data/imgs/final_imgs_256/", 1)
            changed += 1
    # 写回
    with open(p, "w", encoding="utf-8", newline="") as f:
        ...
        w.writeheader()
        w.writerows(rows)
    print(f"{name}: {len(rows)} rows, {changed} paths fixed -> data/imgs/final_imgs_256/")
    # 验证第一行文件存在
