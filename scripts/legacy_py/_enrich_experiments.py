...
def main():
    ...
    for p in sorted(glob.glob("src/train/configs/*.json")):
        ...
        # results_dir 形如 assets/results/<series>
    ...
    # ── 3. 合并 ───────────────────────────────────────────────────────────
    exps = load_csv("assets/experiments.csv")
    out = []
    for e in exps:
        series = e.get("series", "")
    ...
    with open("assets/experiments_enriched.csv", "w", encoding="utf-8",
              newline="") as f:
        w = csv.DictWriter(f, fieldnames=cols, extrasaction="ignore")
        w.writeheader()
        w.writerows(out)
    print(f"# wrote assets/experiments_enriched.csv ({len(out)} rows)")
