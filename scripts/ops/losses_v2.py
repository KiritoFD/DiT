...
#
# 旧版（已删除）的两个致命问题：
#   1) SkeletonLoss: 用 3x3 膨胀的 1px 骨架当 "背景掩码"，而毛笔笔画通常 10~30px
#      宽，笔画主体被误判为背景并遭 off_skel 惩罚 -> 模型被迫抹掉笔画血肉。
#      修复：SkeletonLoss 只做正向牵引（recall），绝不惩罚骨架以外的墨水，
#      笔画粗细与飞白交给扩散损失决定。
#   2) SobelCannyLoss: 逐图除以最大梯度（早期模糊 x0 中噪点被放大成"最强边缘"，
#      梯度尺度失真）；并且用连续梯度场去 L1 拟合 1px 二值 Canny 线，逼模型消灭
#      抗锯齿与晕染。修复：EdgeGradientLoss 不做归一化、不做二值匹配，直接对齐
#      预测图与 GT 图的连续 Sobel 梯度幅值场（超分领域经典 gradient-profile 损失），
#      保边缘锐度同时保留灰度过渡。
# ============================================================================
